...
import 'package:saakouk/view/signup/home/widget/product_widget.dart';
...
class HomeScreen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final controller = Get.put(HomeController(userType: userType));
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        centerTitle: true,
        title: Text(
          'Welcome ${userType == "store" ? "Store Owner" : "Cafe Owner"}',
        ),
        backgroundColor: Colors.transparent,
        surfaceTintColor: Colors.transparent,
        elevation: 0,
      ...
      builder:
          (_) => AlertDialog(
            ...
            actions: [
              ...
              ElevatedButton(
                onPressed: () async {
                  final newRack = rackController.text.trim();
                  if (newRack.isNotEmpty) {
                    await controller.updateRackByProductName(
                      productName,
                      newRack,
                    );
                    Get.back();
